...
2. **Execution Loop**: For iteration `i=1` to `Iterations`:
    - For `CurrentSkill` in `Skills Chain`:
        ...
        *   **Prompt**: `"Run the [CurrentSkill] skill on target '[Target]'.
            ...
            steps: [Active Context / Findings]."` When a **Scratch Dir** was
            supplied, append: `" Your run-scratch sub-path is
        ...
        **Skip-if-present (resume).** When a Scratch Dir is supplied, before
        ...
        context for the subsequent skill in the chain (the subagent has already
        written its durable artifact to the Scratch Dir; the chat report is a
        summary).
