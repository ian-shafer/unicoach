...
3. **Spawn Subagents in Bounded Batches**: Launch a background subagent for each
   ...
   - **Skip-if-present**: when a Scratch Dir is supplied, do **not** spawn a
     ...
     produced a real verdict. Note, too, that the caller (e.g. `rfc-impl-review`
     Phase 2d) may drop a carry-forward **sentinel** at `[Skill Name].json` here
     — a small marker pointing at a prior pass whose verdict is still trusted.
     Treat a sentinel as already-done: do **not** re-spawn that lens, count it
     as present in the step-5 completeness check, and list it in the compiled
