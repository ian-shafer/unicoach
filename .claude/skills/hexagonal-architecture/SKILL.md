...
3. **Adapters Implement Ports**: Outside of the domain, the `adapters/` layer
   implements those ports using specific libraries (e.g., SendGrid, PostgreSQL).
4. **Strict Dependency Rule**: Source code dependencies must point inwards.
   ...
   external framework/ORM, barring specific allowlists (e.g., standard library).
...
1. **Create the Domain Entity** (e.g., `domain/appointment.go`) and its core
   business rules. Do not include database IDs or JSON tags if they leak adapter
   logic (though standard language pragmatism applies).
