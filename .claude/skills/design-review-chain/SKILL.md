...
**Role — orchestrator.** This chain wears the **orchestrator** hat from `iterative-work` (bounded fan-out, per-leaf capture, reconstruct-from-artifacts, resumable via skip-if-present); the `design-review-*` leaves are **workers**. Below is only the chain-specific wiring.
...
2.  **Spawn Subagents in Bounded Batches**: Launch a background subagent for each
    ...
    - **prompt**: `"Run the [Skill Name] skill on target '[Target]' and return
        ...
        FAIL) and the detailed Evaluation.<SCRATCH>"` — where `<SCRATCH>`, when a
        **Scratch Dir** was supplied, is: `" The instant you finish, write your
        verdict (Status + Evaluation) to
