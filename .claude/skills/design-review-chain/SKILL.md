...
description: >-
  ...
  `design-review-*` leaf agents — one per micro-skill — then compiles their
...
3. **Spawn Subagents in Bounded Batches**: Launch a background subagent for each
   ...
   - **prompt**:
     `"Apply the [Skill Name] skill to the change described in the
        ...
        Status (PASS or FAIL) and the detailed Evaluation.<SCRATCH>\n\n=== REVIEW
        CONTEXT (target '[Target]', base '[Base Revision]') ===\n<review-context>"`
