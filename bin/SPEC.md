...
- **Required env**: `POSTGRES_PORT` is required (no in-code default);
  ...
  `PORT`. `PUBLIC_WEB_PORT` (default `8082`, defined in `.env`) is the
  public-web daemon's bind port, read by
  `public-web-check`/`public-web-wait-for-health` (and, JVM-side, a required
  substitution in service.conf's email verify-link default), and is likewise
