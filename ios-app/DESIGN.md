...
- The menu holds: **New conversation**, the **recent conversations** in the
  server's MRU order, and a pinned footer of the non-chat surfaces: **My
  colleges** (RFC 137), **All conversations**, and **Settings**.
...
| College-list surfaces       | RFC 137. The list reuses the conversation-row card; the **status pill** is a monochrome capsule — `dsOverline` uppercase text inside a 1pt `FieldBorder` hairline, no fill and no new colour role (a status is metadata, not an alert). The detail screen's reasons editor is the §5 text-field box grown multiline (label above, `Surface` fill, hairline border); supporting observations are read-only quoted `dsCaption` cards. The add picker's result rows are the same card with a "City, ST" `dsCaption` second line.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
