...
| Rendered Markdown           | The coach's turn is rendered Markdown (`Markdown/MarkdownView.swift`); the student's stays plain `Text`. Headings top out at `dsTitle` (`dsDisplay` is a _screen_ heading); code is `dsCode` in an outlined `DSRadius.control` box that scrolls rather than wraps; a quote is a leading 1pt `FieldBorder` rule, not a tinted wash; a table is a `Grid` in a horizontal `ScrollView`, header in `dsLabel`, rows separated by the same hairline, no fills and no zebra striping. Links are `TextPrimary` **underlined**, never an accent (§6). `DSMarkdown` holds the three measurements Markdown adds — the list marker column and the table column floor and ceiling. |
