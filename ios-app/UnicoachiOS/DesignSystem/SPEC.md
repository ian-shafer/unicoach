...
- **Token-only styling.** Every component in this module MUST derive its colors,
  ...
  `Theme.swift` (`Color` extension, `Font` extension, `DSSpacing`, `DSRadius`). A
  component NEVER hardcodes a color literal, a fixed font/point size, a raw
...
- **Color via asset-catalog resolution.** Every color token MUST resolve through
  a **named asset-catalog Color Set** (`Color("<Name>", bundle: .main)`), never a
  literal RGB/`Color(red:green:blue:)`. Dark-mode and light-mode values are
...
- **Presentation purity.** Components MUST communicate with callers only through
  ...
  and MUST NOT reach for application state, services, or navigation. This keeps
  the module reusable and independently previewable.

- **Caller-owned focus.** `LabeledField` MUST NOT own focus state. It is generic
  over the caller's focus enum (`Value: Hashable`) and binds to a caller-supplied
  `FocusState<Value?>.Binding` plus an `equals` value, so the caller retains full
  control of tab order and submit chaining.
...
- **Loading implies disabled.** While `LoadingButton.isLoading` is `true`, the
  ...
  it NEVER presents a tappable titled button in the loading state.
...
- **iOS-only.** The module targets iOS exclusively (it uses iOS-specific types
  ...
  introduces cross-platform `#if os(...)` compile guards around its styling;
  the iOS deployment target is assumed, so `LabeledField` applies its
  caller-supplied input traits unconditionally, with no platform branch.

- **VoiceOver announces error text.** `FieldErrorText` and `FormErrorBanner` MUST
  combine their children into a single accessibility element whose label is the
  error `message`, so VoiceOver announces the message as one utterance.
...
- **`LoadingButton: View`** —
  ...
  `ProgressView`. The three accessibility parameters follow the opt-in invariant.
- **`LabeledField<Value: Hashable>: View`** —
  ...
  onSubmit: = {})`. Renders a `dsLabel`/`dsTextSecondary` label, a `TextField` or
  (`isSecure`) `SecureField` styled with `dsSurface` background, `DSRadius.field`
  radius, `dsBody`/`dsTextPrimary` input, and a border that is `dsFieldBorder`
  normally / `dsError` on error. Accessibility identifier/label are applied to
  the input control. Default `submitLabel` is `.next`; callers pass a terminal
  label (`.go`/`.done`) for their last field.
- **`FieldErrorText: View`** — `init(_ message:)`. Inline error: `dsError` color,
  `dsCaption` font, an `exclamationmark.circle` SF Symbol, VoiceOver-announced.
- **`FormErrorBanner: View`** — `init(_ message:)`. General (non-field) error: a
  tonal `dsError`-on-`dsSurface` banner with a leading `exclamationmark.triangle.fill`
  SF Symbol and `dsCaption` message text, `DSRadius.field` radius, VoiceOver-announced.

### Error handling

- This module performs **no error handling logic**. It only *renders* error
...
- **Toolchain:** Built and previewed with **system Xcode** (`/usr/bin/xcodebuild`,
  Xcode 26.2), **not** the project's Nix dev shell. The `nix develop -c` rule in
  `CLAUDE.md` applies to the Kotlin/Postgres toolchain only and does **not** wrap
  iOS build commands.
...
- **Asset dependency:** Color tokens resolve named Color Sets that live in
  ...
  hex values and their WCAG AA verification are owned there; this module owns only
  the token-name → Color-Set-name binding.
