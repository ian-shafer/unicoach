...
Updated: 2026-09-08 — **RFC 175, `shape/07b/borrowing`** (`main@3924931d` +
`64b4875a`): a family can now see **what graduates of a school actually
borrowed, including private loans** — the half the Scorecard's federal-only debt
figure never shows. Migration 0089 adds **no table**: five measures, three aid
scopes and six populations extend the canonical store's CHECK lists, so loan
type rides in the measure for an average and in the population for a count.
Coach prompt **v21** (migration 0090); rollback =
`COACHING_SYSTEM_PROMPT_VERSION=v20`. **Next free RFC 176; next free migration
0091.** With `shape/07b` landed, `shape/05/search-on-your-price` (rfc-169, in
flight) is the last slice standing between here and `shape/08`.

## TL;DR — next steps, most important first

1. **A FAMILY CAN NOW SEE WHAT GRADUATES HERE ACTUALLY BORROWED, INCLUDING
...
2. **A FAMILY CAN NOW FIX ITS OWN MONEY FACTS ON A SCREEN (RFC 171,
...
3. **THE COACH CAN NOW ANSWER "DO WE HAVE TO DO THE CSS PROFILE?" (RFC 170,
...
4. **A FAMILY CAN NOW SEE THE CANONICAL MONEY STORE (RFC 166,
...
5. **UNICOACH NOW KNOWS WHO AN AID NUMBER IS ABOUT (RFC 162,
...
6. **THE iOS COLLEGE LIST NOW SAYS WHAT IT MEANS ON THE WIRE (RFC 168,
...
7. **FORMS CAN STOP GUESSING: ONE ENDPOINT SERVES THE VOCABULARY (RFC 165,
...
8. **THE IN-DISTRICT PRICE IS REAL NOW (RFC 161, `shape/02/ipeds-ic-ay`,
...
9. **THE MONEY STORE IS SHAPED LIKE MONEY (RFC 158, `shape/01/canonical-store`,
...
10. **BEAT 1 IS COMPLETE: the coach now asks to share the Family Cost Report, at
...
11. **FIRST BRIEF 0006 SLICE LANDED: the coach now answers Pell and loan
...
12. **BRIEF 0006 — MONEY IN UNICOACH SHAPE — GATES 1+2 APPROVED (Ian,
...
13. **A NUMBER IN THE PARENT'S REPORT WAS NOT THE FAMILY'S NUMBER, AND IAN FOUND
...
14. **THE FAMILY COST REPORT IS LIVE. `first-value/05/family-cost-report` (S5)
...
15. **Brief 0003 — clear money language — COMPLETE. `money/04/where-youll-live`
...
16. **Brief 0004 — college search index — CORE COMPLETE. Every slice has landed
...
17. **Brief 0001 S4 COMPLETE — S4a (RFC 140) and S4b (RFC 148, 2026-08-30).**
...
18. **Before any App Store submission: brief 0002, account deletion** — parked
...
0006, 2026-09-05).** `shape/04/cost-answers-from-canonical` (RFC 166) and
`shape/07a/need-and-forms` (RFC 170) have both LANDED and leave the board. That
makes `shape/05/search-on-your-price` the **last remaining reader** of the
publisher-shaped money columns on `colleges`, and it is IN FLIGHT (rfc-169).
...
`shape/07a` and `shape/07b/borrowing`, and **both have now LANDED** (RFCs 170
and 175), so neither is board material any longer.
...
### How soft is this number? (brief 0008) — wave 1 startable

Gates 1+2 approved 2026-09-05. Paste one of these:

    start work on soft/01/one-hedge-seam

    start work on soft/04/two-more-ingest-refusals

`soft/02/assurance-tiers` is BLOCKED on `soft/01` (the seam is its only reader).
`soft/03/the-year-we-cite` is READY on the board but held by hand until
`shape/07b/borrowing` (rfc-175) lands — both edit `CanonicalMoneyLoader.kt`.
