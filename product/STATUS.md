...
Updated: 2026-09-09 — **RFC 182, `soft/04/two-more-ingest-refusals`**
(`main@6b7586cf` + `3787f936`): `bin/fetch-cds-seed` now checks a Common Data
Set filing against something OUTSIDE itself for the first time. Two refusals, in
the same drop-and-fall-back shape as the fourteen already there — a freshman
count that disagrees with the federal file by more than 50% either way (CDS
`H.201` vs IPEDS `SCFA1N`, plus an `H.204` sibling on the aid-policy doc), and
one filing attributed to several colleges (a value signature repeating
byte-for-byte across different `unit_id`s in one `source_year`). IPEDS arrives
through a new `-I` option; an absent `sfa2223.csv` SKIPS the check and says so
in `PROVENANCE.json`, a present-but-unreadable one is a fault. Drop counts:
`merit_freshmen_vs_ipeds_implausible` 15,
`aid_policy_freshmen_vs_ipeds_implausible` 7,
`filing_attributed_to_multiple_units` 35. Seed rows `merit-aid` 368 -> 352,
`aid_policy` 3,568 -> 3,496, `admission-factors` 373 -> 369, `deadlines` 846
-> 831. **No table, no migration, no family-visible flag** (gate-1 D4, standing
D7) — a refusal says "we could not read this filing", which is our gap and is
never spoken as the school's. Note this line is written **after the fact**: RFC
182's run landed the code and never wrote the product docs, so `slice-board`
read `soft/04` as READY for a day. **Next free RFC 186** (`rfc/` tops out at
184; `pipeline/rfc-178`, `-183` and `-185` are live runs — recompute this, never
copy it). **Migrations no longer have numbers** (RFC 180, `main@9343985c`): a
new migration is `<kebab-slug>.sql` with its apply order declared as a line in
`db/schema/ORDER`, appended at land time by `ship-order` — the old "next free
migration NNNN" reading here is retired. Brief 0008 has ONE slice left,
`soft/03/the-year-we-cite`, in flight as `pipeline/rfc-183`. **Brief 0006 is
closed**: `shape/08/drop-the-publisher-shape` landed as RFC 176 (2026-09-09) and
the publisher money columns are gone from `colleges` and `colleges_versions`;
only the deferred `shape/09/exchange-participation` remains.

## TL;DR — next steps, most important first

1. **A CDS FILING IS NOW CHECKED AGAINST THE FEDERAL FILE (RFC 182,
...
2. **A FIGURE NOW SAYS WHAT KIND OF NUMBER IT IS (RFC 179,
...
3. **A FIGURE'S PUBLISHER IS NOW DERIVED, AND THE LIE IS GONE (RFC 177,
...
4. **SEARCH NOW RANKS ON THE PRICE YOU WOULD ACTUALLY PAY (RFC 169,
...
5. **A FAMILY CAN NOW SEE WHAT GRADUATES HERE ACTUALLY BORROWED, INCLUDING
...
6. **A FAMILY CAN NOW FIX ITS OWN MONEY FACTS ON A SCREEN (RFC 171,
...
7. **THE COACH CAN NOW ANSWER "DO WE HAVE TO DO THE CSS PROFILE?" (RFC 170,
...
8. **A FAMILY CAN NOW SEE THE CANONICAL MONEY STORE (RFC 166,
...
9. **ONE SOURCE FOR EVERY MONEY NUMBER (RFC 176,
...
10. **UNICOACH NOW KNOWS WHO AN AID NUMBER IS ABOUT (RFC 162,
...
11. **THE iOS COLLEGE LIST NOW SAYS WHAT IT MEANS ON THE WIRE (RFC 168,
...
12. **FORMS CAN STOP GUESSING: ONE ENDPOINT SERVES THE VOCABULARY (RFC 165,
...
13. **THE IN-DISTRICT PRICE IS REAL NOW (RFC 161, `shape/02/ipeds-ic-ay`,
...
14. **THE MONEY STORE IS SHAPED LIKE MONEY (RFC 158, `shape/01/canonical-store`,
...
15. **BEAT 1 IS COMPLETE: the coach now asks to share the Family Cost Report, at
...
16. **FIRST BRIEF 0006 SLICE LANDED: the coach now answers Pell and loan
...
17. **BRIEF 0006 — MONEY IN UNICOACH SHAPE — GATES 1+2 APPROVED (Ian,
...
18. **A NUMBER IN THE PARENT'S REPORT WAS NOT THE FAMILY'S NUMBER, AND IAN FOUND
...
19. **THE FAMILY COST REPORT IS LIVE. `first-value/05/family-cost-report` (S5)
...
20. **Brief 0003 — clear money language — COMPLETE. `money/04/where-youll-live`
...
21. **Brief 0004 — college search index — CORE COMPLETE. Every slice has landed
...
22. **Brief 0001 S4 COMPLETE — S4a (RFC 140) and S4b (RFC 148, 2026-08-30).**
...
22. **Before any App Store submission: brief 0002, account deletion** — parked
...
**Brief 0008 is three-quarters landed (2026-09-09).**
`soft/04/two-more-ingest-refusals` LANDED as RFC 182 and leaves the board, after
`soft/02/assurance-tiers` (RFC 179) and `soft/01/one-hedge-seam` (RFC 177). All
three wave-1 and wave-2 slices are gone, and **none of them blocked anything**,
so their exits unblock nothing. What remains is `soft/03/the-year-we-cite`
alone, now **IN FLIGHT as `pipeline/rfc-183`**. Brief 0008 has nothing startable
left.
...
### How soft is this number? (brief 0008) — three of four landed, nothing to start

Gates 1+2 approved 2026-09-05. `soft/01/one-hedge-seam` (RFC 177),
`soft/02/assurance-tiers` (RFC 179) and `soft/04/two-more-ingest-refusals`
(RFC 182) have LANDED — do not start any of them again. The paste-ready prompt
for `soft/04` was removed once its close-out was written, because leaving it
here is exactly how a landed slice gets dispatched a second time.

The last slice, **`soft/03/the-year-we-cite`, is IN FLIGHT** as
`pipeline/rfc-183` — check `slice-board` rather than starting it. It CONFLICTS
with the landed `soft/02` on `CanonicalMoneyLoader.kt`; that is file overlap and
a rebase, never an order.

Debt this brief has queued for /chart, if you are picking it up: a
`PublishedCell` value type for the `(source, source_variable)` pair (declined in
RFC 179, asked for by three review lenses, crosses four modules and wants its
own slice — note RFC 184 has since landed a first cut, so re-check the tree
before specifying it), and the live extractor defect RFC 182 found and no slice
owns — Boston University and Carnegie Mellon both report
`graduating_class_count = 2025`, a class YEAR read as a headcount, landed by
`shape/07b`.
