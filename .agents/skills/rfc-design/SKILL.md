...
- **Architectural Decisions**:
  - **Interactive Mode**: When making important architectural decisions (like whether a database table should be versioned), you MUST ask the Architect. Always provide multiple options and a clear recommendation.
  - **Non-Interactive Mode**: Document the decision INLINE within the artifact. Present multiple options with a clear recommendation, and explicitly flag it with `REQUIRES ARCHITECT DECISION` so it can be addressed at the human gate. Do NOT leave sections blank waiting for a response.
