...
1. **Implement & Test**: Implement the required code for the current step,
   including all necessary test code.
2. **Demonstrate Success**: Once the implementation is complete and the test
...
3. **Architect Review**: Allow the architect to review the code changes you have
   made.
4. **Iterate**: Iterate with the architect on any desired code changes,
   refinements, or fixes.
5. **Await LGTM**: Once all code changes are implemented and tests pass, wait
   for an explicit "LGTM" (Looks Good To Me) approval from the architect. Do not
   proceed until this is received.
6. **Propose Commit**: Once the LGTM is received for the code, propose a commit
   message by writing it to a temporary file (e.g., in `/tmp/`) and allow the
   architect to review and edit it if desired. The commit message **must**
   include the spec file name and the specific implementation plan step being
   completed.
7. **Commit Code**: Once the architect gives the LGTM on the commit message,
   commit the code using the approved message.
