-- Seed the v15 coach system prompt into the immutable system_prompts catalog
-- (RFC 155). v13 taught the coach to answer "what other schools are like this
-- one" from the similar_colleges tool, and v14 to lead with the one way of
-- living the family actually plans on -- but everything those paragraphs
-- produce stays inside the student's chat window: the person who actually
-- writes the cheque never sees it. RFC 155 lands the Family Cost Report -- a
-- student-initiated, revocable, tokenized web page a parent opens with no
-- login -- together with the share_cost_report and revoke_cost_report_share
-- tools. On 0047's precedent (a capability advertised but not prompted goes
-- unused in conversation), the coach needs the policy in the prompt too.
--
-- This body is v14 VERBATIM plus exactly one appended paragraph -- the additive
-- shape of 0047 (v2->v3), 0048 (v3->v4), 0058 (v7->v8), 0063 (v8->v9), 0065
-- (v9->v10), 0066 (v10->v11), 0068 (v11->v12), 0069 (v12->v13) and 0072
-- (v13->v14). It is correct here for the same reason it was there: nothing in
-- the money, college-list, admissions, living-arrangement, search, comparison,
-- find_college, similar_colleges or living-plan paragraphs changes meaning, and
-- an append preserves RFC 142's source-jargon sentence, RFC 141's glossary
-- pairs, RFC 151's comparison paragraph, RFC 154's name-lookup paragraph, RFC
-- 153's similar-colleges paragraph and RFC 152's living-plan paragraph
-- byte-identically at their interior positions for free. The leading space on
-- the first appended chunk is the single space joining the paragraphs, so the
-- whole v14 body stays a byte-identical prefix.
--
-- The paragraph carries RFC 155's rules IN FULL: the report is offered only
-- after a cost comparison has ACTUALLY happened (value before ask, brief 0001
-- D12); it is never sent without the student asking or agreeing; handing over
-- the link is always accompanied by the plain statement that anyone holding it
-- can see the list and that the student can revoke it at any time; a decline
-- ends the subject and changes nothing about what the coach does next (the
-- nudge and its commitment trigger belong to first-value/06, not here);
-- revocation is spoken as what it is -- every link the student has shared is
-- dead; asking again returns the SAME link, so one already sent keeps working;
-- and the page's liveness is stated rather than left to be discovered, because
-- a parent who thinks they hold a fixed document will read last week's list as
-- this week's answer, and a student must know the visibility is ongoing rather
-- than a one-time send.
--
...
-- price arithmetic at all, so the served-body guard in SystemPromptCatalogTest
-- keeps passing, and it transcribes no source code.
--
-- Mirrors 0011/0026/0037/0044/0047/0048/0049/0050/0053/0058/0063/0065/0066/
-- 0068/0069/0072: architect-approved copy stored as a single concatenated
...
-- row. The v14 row remains in the catalog (immutable-entity design, 0007),
-- which makes a rollback one env var: COACHING_SYSTEM_PROMPT_VERSION=v14.
-- service.conf pins v15.
