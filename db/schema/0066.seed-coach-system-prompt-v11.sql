-- Seed the v11 coach system prompt into the immutable system_prompts catalog
-- (RFC 151). v10 taught the coach to lead with one school's price split by
-- living arrangement and to search by subject, but a side-by-side is where cost
-- advice either earns trust or quietly lies: a dollar figure is a statistic
-- about a population, a year, a residency and a way of living, and a bare
-- column label hides all four. RFC 151 lands `comparison_basis` on every cost
-- result carrying two or more colleges; on 0047's precedent (a capability
-- advertised but not prompted goes unused in conversation), the coach needs the
-- policy in the prompt too.
--
-- This body is v10 VERBATIM plus exactly one appended paragraph — the additive
-- shape of 0047 (v2->v3), 0048 (v3->v4), 0058 (v7->v8), 0063 (v8->v9) and 0065
-- (v9->v10). It is correct here for the same reason it was there: nothing in
-- the money, college-list, admissions, living-arrangement or search paragraphs
-- changes meaning, and an append preserves RFC 142's source-jargon sentence,
-- RFC 141's glossary pairs and RFC 150's search paragraph byte-identically at
-- their interior positions for free. The leading space on the first appended
-- chunk is the single space joining the paragraphs, so the whole v10 body stays
-- a byte-identical prefix.
--
-- The paragraph carries RFC 151's rules IN FULL: when two or more schools appear
-- together, the five basis lines are said FIRST, as ordinary copy above the
-- table rather than as a note beneath it; tuition and fees — the price the
-- school sets and publishes — is rendered above the estimated living costs and
-- the two blocks are named; rows are schools and the table stays inside RFC
-- 124's three-column cap or becomes a list; a part a school does not report is a
-- labelled blank, never a zero, never a neighbour's number, and never summed
-- into a total; a school with no residence halls has none rather than being
-- unreported (RFC 149 D-B, restated in the comparison case); and one column
-- holds one residency and one way of living, with loans, work-study and a net
-- price never subtracted from any figure in it.
--
...
-- swept): it says housing and food, tuition and fees, the published price and a
-- financial aid offer, and it names none of the retired terms. Every occurrence
-- of "subtract" in it is immediately preceded by "never ", so the served-body
-- guard in SystemPromptCatalogTest keeps passing, and it transcribes no source
-- code.
--
-- Mirrors 0011/0026/0037/0044/0047/0048/0049/0050/0053/0058/0063/0065:
...
-- seed is never edited; a later change is a new version row. The v10 row remains
-- in the catalog (immutable-entity design, 0007), which makes a rollback one env
-- var: COACHING_SYSTEM_PROMPT_VERSION=v10. service.conf pins v11.
