-- Seed the v17 coach system prompt into the immutable system_prompts catalog
-- (RFC 159). v16 gave the coach every price a college publishes; none of it
-- answers the two questions a family asks that need NO college data at all --
-- "do we qualify for a Pell Grant, roughly?" and "what would loans look
-- like?". RFC 159 lands the federal_aid_policy tool over the migration-seeded
-- policy_parameters store; this paragraph tells the coach how to narrate what
-- it serves.
--
-- This body is v16 VERBATIM plus exactly one appended paragraph -- the
-- additive shape of 0047 (v2->v3) through 0076 (v15->v16). It is correct here
-- for the same reason it was there: nothing in the earlier paragraphs changes
-- meaning, and an append preserves every prior version's copy byte-identically
-- at its interior position for free. The leading space on the first appended
-- chunk is the single space joining the paragraphs, so the whole v16 body
-- stays a byte-identical prefix.
--
-- The paragraph carries RFC 159 D-D/D-F/D-G in full: Pell and loan questions
-- are answered from the federal_aid_policy tool, never memory; every figure is
-- said with its award year and attributed to the cited Federal Student Aid
-- source; Pell is framed as eligibility and a range, never a promised amount;
-- a loan limit is a cap the family repays with interest and is never
-- subtracted from any price (the one "subtract" is prefixed "never", per the
-- served-body guard); figures a tool marks as a prior award year's are said
-- so plainly; and the dependency question is invited at most once, with its
-- value named (which loan limits and which Pell income test apply), declinable
-- with a full answer served regardless.
--
-- One wording note: "award year" is deliberate and is NOT the retired
-- "award" of RFC 141's glossary (a financial aid offer). The federal award
-- year is the July-June policy year FSA itself names, and citing it is the
-- slice's first acceptance criterion; no aid offer is called an award here.
--
-- Mirrors 0011/0026/0037/0044/0047-0076: architect-approved copy stored as a
-- single concatenated string (|| is layout only; the body is verbatim and
-- untrimmed, so no newlines are introduced). This seed is never edited; a
-- later change is a new version row. The v16 row remains in the catalog
-- (immutable-entity design, 0007), which makes a rollback one env var:
-- COACHING_SYSTEM_PROMPT_VERSION=v16. service.conf pins v17.
