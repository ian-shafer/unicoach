-- Seed the v10 coach system prompt into the immutable system_prompts catalog
-- (RFC 150). v9 taught the coach to talk about the published price split by
-- living arrangement. What it could not do is USE the search tool honestly:
-- RFC 150 rebuilds `search_colleges` onto `college_search_index`, which
-- replaces the guessed federal program code with a `subject` WORD, adds the
-- IPEDS attribute filters, and returns `excluded_unknown` and `source_years`
-- beside `total_matches`. On 0047's precedent (a capability advertised but not
-- prompted goes unused in conversation), the coach needs the policy in the
-- prompt too.
--
-- This body is v9 VERBATIM plus exactly one appended paragraph -- the additive
-- shape of 0047 (v2->v3), 0048 (v3->v4), 0058 (v7->v8) and 0063 (v8->v9). It is
-- correct here for the same reason it was there: nothing in the money,
-- college-list or admissions paragraphs changes meaning, and an append
-- preserves RFC 142's source-jargon sentence, RFC 141's glossary pairs and RFC
-- 149's cost-component paragraph byte-identically at their interior positions
-- for free. The leading space on the first appended chunk is the single space
-- joining the paragraphs, so the whole v9 body stays a byte-identical prefix.
--
-- The paragraph carries RFC 150 D58 IN FULL: search by SUBJECT, never by a raw
-- code, because the subject word is expanded to the real program codes and a
-- guessed code can match nothing while looking like it matched; report
-- `total_matches` when saying how many schools match, and report the
-- excluded-unknown counts IN WORDS, because a school that does not report an
-- axis was not judged on it and is not a "no"; never state a source's own code
-- to a student. Brief 0003's money vocabulary applies VERBATIM to any price the
-- tool returns -- tuition and fees, housing and food, the published price, a
-- financial aid offer -- and loans are never subtracted from a price.
--
-- Mirrors 0011/0026/0037/0044/0047/0048/0049/0050/0053/0058/0063:
-- architect-approved copy stored as a single concatenated string (|| is layout
-- only; the body is verbatim and untrimmed, so no newlines are introduced).
-- This seed is never edited; a later change is a new version row. The v9 row
-- remains in the catalog (immutable-entity design, 0007), which makes a
-- rollback one env var: COACHING_SYSTEM_PROMPT_VERSION=v9. service.conf pins
-- v10.
