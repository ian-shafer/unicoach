-- Seed the v13 coach system prompt into the immutable system_prompts catalog
-- (RFC 153). v12 gave the coach a way to turn a school the student NAMED into
-- a college_id (`find_college`) and kept `search_colleges` for the kind of
-- school a student wants. Neither answers the third question a student asks
-- about a school they have already named: "what else is like it?". RFC 153
-- lands the `similar_colleges` tool, which takes an anchor `college_id` and
-- ranks real peers on a per-call choice of axes; on 0047's precedent (a
-- capability advertised but not prompted goes unused in conversation), the
-- coach needs the reading rule in the prompt too.
--
-- This body is v12 VERBATIM plus exactly one appended paragraph -- the additive
-- shape of 0047 (v2->v3), 0048 (v3->v4), 0058 (v7->v8), 0063 (v8->v9), 0065
-- (v9->v10), 0066 (v10->v11) and 0068 (v11->v12). It is correct here for the
-- same reason it was there: nothing in the money, college-list, admissions,
-- living-arrangement, search, comparison or name-lookup paragraphs changes
-- meaning, and an append preserves RFC 142's source-jargon sentence, RFC 141's
-- glossary pairs, RFC 150's search paragraph, RFC 151's comparison paragraph
-- and RFC 154's name-lookup paragraph byte-identically at their interior
-- positions for free. The leading space on the first appended chunk is the
-- single space joining the paragraphs, so the whole v12 body stays a
-- byte-identical prefix.
--
-- The paragraph carries RFC 153's THIRD tool role and nothing else: a student
-- who names one school and asks for others like it is answered from the
-- `similar_colleges` call rather than from peers the model remembers; the tool
-- is given an anchor `college_id`, which is `find_college`'s job to produce --
-- this paragraph claims NO name resolution of its own, so there is exactly one
-- name-to-id door in the prompt; the axes the answer ranked on and the axes it
-- dropped are both said with their reasons, because an unreported figure was
-- not judged and a silence is never a no; each school's `axes_scored` is read
-- aloud, because a school judged on one axis is a weaker claim than one judged
-- on three; and `distance` is a RANK AID, never a percentage and never a
-- quality score.
--
...
-- swept): it names none of the retired money terms, and every occurrence of
-- "subtract" in the served body still sits behind "never ", so the guard in
-- SystemPromptCatalogTest keeps passing. It transcribes no source code.
--
-- Mirrors 0011/0026/0037/0044/0047/0048/0049/0050/0053/0058/0063/0065/0066/0068:
...
-- seed is never edited; a later change is a new version row. The v12 row remains
-- in the catalog (immutable-entity design, 0007), which makes a rollback one env
-- var: COACHING_SYSTEM_PROMPT_VERSION=v12. service.conf pins v13.
