-- Seed the v18 coach system prompt into the immutable system_prompts catalog
-- (RFC 160). v15 taught the coach to offer the Family Cost Report only after a
-- cost comparison and never to open with it. RFC 160 adds the one sanctioned
-- exception: the synthesis pass now writes a share-nudge commitment
-- (lens 'share_report') for an eligible student, and that reflection rides the
-- next-session opener. When it does, raising the offer once, naturally, IS the
-- sanctioned moment -- the coach is acting on a surfaced reflection, not
-- opening unasked. A decline or a deferral closes the topic for that
-- conversation without residue, nothing is ever gated on sharing (brief 0001
-- D11), and a student who says they never want the suggestion again is
-- answered with the new stop_cost_report_offers tool, whose 'opted_out' share
-- event suppresses the nudge forever.
--
-- This body is v17 VERBATIM plus exactly one appended paragraph -- the additive
-- shape of 0047 (v2->v3) through 0079 (v16->v17). Nothing in the interior
-- paragraphs changes meaning, so every earlier RFC's copy is preserved
-- byte-identically at its interior position for free; the leading space on the
-- first appended chunk is the single space joining the paragraphs, so the whole
-- v17 body stays a byte-identical prefix.
--
-- Landed after RFC 159's v17 (both runs were open concurrently and both
-- appended to v16); this seed composes on the body that actually reached main,
-- so the federal-aid paragraph and the share-nudge paragraph both stand.
