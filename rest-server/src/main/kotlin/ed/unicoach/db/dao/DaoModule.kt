...
sealed interface FindResult {
    ...
    data class DatabaseFailure(val msg: String) : FindResult
}

sealed interface CreateResult {
    data class Success(val user: User) : CreateResult
    data object DuplicateEmail : CreateResult
    data class ConstraintViolation(val reason: String) : CreateResult
    data class DatabaseFailure(val msg: String) : CreateResult
}
...
sealed interface UpdateResult {
    ...
    data class ConstraintViolation(val reason: String) : UpdateResult
    data class DatabaseFailure(val msg: String) : UpdateResult
}

sealed interface DeleteResult {
    data class Success(val user: User) : DeleteResult
    data object NotFound : DeleteResult
    data object ConcurrentModification : DeleteResult
    data class DatabaseFailure(val msg: String) : DeleteResult
}

sealed interface FindVersionResult {
    data class Success(val version: UserVersion) : FindVersionResult
    data object NotFound : FindVersionResult
    data class DatabaseFailure(val msg: String) : FindVersionResult
}
