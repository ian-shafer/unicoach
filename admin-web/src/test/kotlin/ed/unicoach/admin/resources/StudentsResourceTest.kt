...
class StudentsResourceTest {
  ...
  fun `user page renders the three coaching-memory panels each linking to canonical detail URLs`() =
    testApplication {
      ...
      // convoId compacts via the panel projection path. This panel's Convo ID column
      // carries no refSlug, so the cell has no navigation glyph (no refPath).
      val convoId = convo.id.value.toString()
      AdminTestSupport.assertCompactUuid(body, convoId)
    }
