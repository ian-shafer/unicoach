...
class FitLensSweepHandlerTest {
  ...
  private fun seedStudent(): String {
    ...
    connection.createStatement().use { stmt ->
      stmt.execute(
        "INSERT INTO users (id, email, name, password_hash) VALUES ('$userId', 'flsweep-$userId@test.com', 'FL Sweep', 'ahash')",
      )
      stmt.execute(
        """
  ...
  private fun countFitLensJobs(): Int =
    connection
      .prepareStatement("SELECT COUNT(*) FROM jobs WHERE job_type = ?")
      .use { stmt ->
        stmt.setString(1, JobType.FIT_LENS.value)
  ...
  @Test
  fun `config advertises FIT_LENS_SWEEP and executionTimeout under lockDuration`() {
    val handler = FitLensSweepHandler(database, QueueService(database))
    assertEquals(JobType.FIT_LENS_SWEEP, handler.jobType)
    assertTrue(
  ...
  @Test
  fun `with N active students the sweep enqueues N FIT_LENS jobs and succeeds`() =
    runTest {
      ...
      assertEquals(3, countFitLensJobs(), "One FIT_LENS job per active student")
    }
  ...
  fun `no active students yields Success and zero enqueues`() =
    runTest {
      ...
      assertTrue(result is JobResult.Success, "Expected Success, got: $result")
      assertEquals(0, countFitLensJobs(), "No students -> no enqueues")
    }
  ...
  fun `a single enqueue failure is logged and does not abort the sweep`() =
    runTest {
      ...
      val handler = FitLensSweepHandler(database, QueueService(database, flakyDao))
      val result = handler.execute(JsonObject(emptyMap()))

      assertTrue(result is JobResult.Success, "A single enqueue failure must not fail the sweep, got: $result")
      assertEquals(2, countFitLensJobs(), "The other two students are still enqueued")
    }
}
