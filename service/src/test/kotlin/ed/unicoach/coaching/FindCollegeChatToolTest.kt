...
class FindCollegeChatToolTest {
  ...
  private val wrapped = FindCollegeTool(CollegeSearchService(database))
  private val adapter = FindCollegeChatTool(wrapped)

  @Test
  fun `name and definition come from the wrapped tool`() {
    assertEquals(FindCollegeTool.TOOL_NAME, adapter.name)
