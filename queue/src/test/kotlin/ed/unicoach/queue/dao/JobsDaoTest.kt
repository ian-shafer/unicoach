...
class JobsDaoTest {
    ...
    private fun simplePayload(tag: String = "test"): JsonObject =
        JsonObject(mapOf("tag" to JsonPrimitive(tag)))
    ...
    fun `insert creates job with SCHEDULED status and default scheduled_at`() {
        ...
        val delta = java.time.Duration.between(job.scheduledAt, Instant.now()).abs()
    ...
    fun `findNextScheduledJob uses SKIP LOCKED to prevent double pickup`() {
        ...
        val config = ed.unicoach.common.config.AppConfig.load("common.conf", "db.conf").getOrThrow()
        val dbConfig = DatabaseConfig.from(config).getOrThrow()
    ...
    fun `insertAttempt records attempt with all fields`() {
        ...
        val result = jobsDao.insertAttempt(
            session = session,
            jobId = claimed.id,
            attemptNumber = 1,
