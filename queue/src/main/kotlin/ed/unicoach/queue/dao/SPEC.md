...
This package is the **queue data access layer**. It owns all SQL interactions
with the `jobs` and `job_attempts` PostgreSQL tables. `JobsDao` is the sole
public class; `Results.kt` defines the sealed result types returned by every
`JobsDao` method. No business logic lives here — only query execution, row
mapping, and error encapsulation.
...
- **I-6**: `finished_at` on `job_attempts` is NEVER set by the application. It
  is set to `NOW()` by the database at insert time.
...
- **I-9**: `reschedule` MUST only transition a job from `RUNNING` → `SCHEDULED`.
  ...
  `JobUpdateResult.InvalidState`.
- **I-10**: `findNextScheduledJob` MUST use
  `SELECT ... FOR UPDATE SKIP LOCKED
  LIMIT 1` to prevent concurrent workers
  from double-claiming the same job.
...
- **I-12**: `deleteByIds` with an empty `ids` list MUST return
  `JobDeleteResult.Success(0)` immediately without executing any SQL.
- **I-13**: `deleteByIds` MUST cascade to `job_attempts` via the
  `ON DELETE
  CASCADE` foreign key constraint on the `job_attempts.job_id`
  column — no explicit deletion of attempts is required in application code.
- **I-14**: `mapJob` MUST use `JobType.fromValue()` for type-safe
  deserialization. An unrecognized `job_type` string in the database MUST cause
  an `IllegalStateException` (propagated via `error()`), wrapped by
  `executeSafely` as `DatabaseFailure`.
...
- **Selection criteria**:
  `status = 'SCHEDULED' AND job_type = ? AND
  scheduled_at <= NOW()`, ordered
  by `scheduled_at ASC`, limit 1.
...
- **Side Effects**: Updates `jobs` row: `status = 'RUNNING'`,
  `locked_until =
  NOW() + lockDuration`. `locked_until` guards against the
  stuck-job reaper prematurely reclaiming the job.
...
- **Side Effects**: Updates `jobs` row: `status = 'COMPLETED'`,
  `locked_until =
  NULL`.
- **Precondition**: Job MUST be in `RUNNING` status.
- **Returns**: Same variants as `claimJob`. `InvalidStateException` if the row
  exists but is not in `RUNNING` status (e.g., already `COMPLETED` or
  `DEAD_LETTERED`). `NotFoundException` only if no row with the given `id`
  exists.
- **Idempotent**: No.

#### `deadLetterJob(session: SqlSession, id: UUID): Result<Job>`

- **Side Effects**: Updates `jobs` row: `status = 'DEAD_LETTERED'`,
  `locked_until
  = NULL`.
- **Precondition**: Job MUST be in `RUNNING` status.
- **Returns**: Same variants as `claimJob`. `InvalidStateException` if the row
...
- **Side Effects**: Updates `jobs` row: `status = 'SCHEDULED'`,
  `scheduled_at =
  NOW() + delay` (SQL interval), `locked_until = NULL`.
- **Precondition**: Job MUST be in `RUNNING` status.
- **Returns**: Same variants as `claimJob`. `InvalidStateException` if not
  `RUNNING`.
- **Idempotent**: No.
...
- **Returns**: `Result.success(attempt)` with the inserted row.
  `Result.failure(error)` on any exception (including constraint violations:
  duplicate `(job_id, attempt_number)`, invalid `status` string, oversized
  `error_message`).
...
- **Returns**: `Result.success(count)`. `Result.failure(error)` on any
  exception.
- **Idempotent**: Yes.
...
- **Returns**: `Result.success(attempts)` — empty list if no attempts exist.
  `Result.failure(error)` on any exception.
- **Idempotent**: Yes.

#### `resetStuckRunning(session: SqlSession): Result<Int>`

- **Side Effects**: Bulk-updates `jobs` rows where
  `status = 'RUNNING' AND
  locked_until < NOW()`: sets `status = 'SCHEDULED'`,
  `locked_until = NULL`. Clock comparison uses database `NOW()`.
...
- **Side Effects**: Physically deletes `jobs` rows where
  `status IN (statuses)
  AND updated_at < NOW() - olderThan` (SQL interval).
  Cascades to `job_attempts`.
- **Empty set guard**: Returns `Result.success(0)` immediately if `statuses` is
  empty.
- **Returns**: `Result.success(count)` — count of deleted rows (may be 0).
  `Result.failure(error)` on any exception.
- **Idempotent**: Yes (repeated deletions on already-deleted rows affect 0
  rows).
...
- **Empty list guard**: Returns `Result.success(0)` immediately if `ids` is
  empty.
- **Returns**: `Result.success(count)`. `Result.failure(error)` on any
  exception.
- **Idempotent**: Yes (deleting already-deleted IDs affects 0 rows).
- **Note**: Also exposed as a vararg overload
  `deleteByIds(session, vararg ids:
  UUID)` for call-site convenience.

#### `findById(session: SqlSession, id: UUID): Result<Job>`

- **Side Effects**: None (read-only).
- **Returns**: `Result.success(job)` if found.
  `Result.failure(NotFoundException)` if no row with the given `id` exists.
  `Result.failure(error)` on any exception.
- **Idempotent**: Yes.
...
- **Returns**: `Result.success(counts)` — a `Map<JobStatus, Int>` containing
  only statuses with at least one row (statuses with zero rows are absent from
  the map). `Result.failure(error)` on any exception.
- **Idempotent**: Yes.
...
All DAO methods return standard Kotlin `Result<T>`. Custom exceptions defined in
`Results.kt` are returned in `Result.failure()` to indicate business logic
errors or expected domain outcomes (e.g., `InvalidStateException`,
`NotFoundException`).
...
- **NOTIFY Trigger**: A trigger on `jobs` emits
  `pg_notify('jobs_channel',
  NEW.job_type)` when a row enters `SCHEDULED`
  status (created by `db/schema/0004.add-jobs-notify-trigger.sql`). `JobsDao`
  does not invoke `NOTIFY` directly — the trigger fires automatically on
  INSERT/UPDATE.
