    ...
    executeSafely(JobInsertResult::DatabaseFailure) {
      ...
      session.prepareStatement(sql).use { stmt ->
        ...
        if (newJob.maxAttempts != null) {
          stmt.setInt(idx++, newJob.maxAttempts)
        } else {
          ...
          if (rs.next()) {
            ...
            if (wasUpdated) {
              ...
              JobUpdateResult.InvalidState(
                "Job [$id] cannot be claimed because it is currently in status [$currentStatus] instead of [SCHEDULED].",
              )
            }
          } else {
        ...
        stmt.executeQuery().use { rs ->
          if (rs.next()) {
            ...
            if (wasUpdated) {
              ...
              val currentStatus = rs.getString("status")
              JobUpdateResult.InvalidState(
                "Job [$id] cannot be updated to [${status.value}] because it is currently in status [$currentStatus] instead of [RUNNING].",
              )
            }
          } else {
            JobUpdateResult.NotFound("Job [$id] not found.")
